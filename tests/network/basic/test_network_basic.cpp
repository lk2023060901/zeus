...
void TestNetworkEvents() {
    ...
    std::vector<NetworkEventType> connection_events = {
        NetworkEventType::CONNECTION_ESTABLISHED, 
        NetworkEventType::CONNECTION_CLOSED
    };
    auto hook_id1 = NetworkEventManager::Instance().RegisterHook(
        connection_events,
        HookInfo("connection_tracker", [](const NetworkEvent& event) {
            std::cout << "Hook: Connection event - " << event.connection_id 
                     << " (" << static_cast<int>(event.type) << ")" << std::endl;
        })
    );
    
    std::vector<NetworkEventType> data_events = {
        NetworkEventType::DATA_SENT, 
        NetworkEventType::DATA_RECEIVED
    };
    auto hook_id2 = NetworkEventManager::Instance().RegisterHook(
        data_events,
        HookInfo("data_tracker", 
